...
export default async function DietChartPrintPage({ params }: Props) {
  ...
  if (!visit || !visit.dietChart) notFound();
  ...
  const chart   = visit.dietChart;
  const patient = visit.patient;
  const age     = calculateAge(patient.dateOfBirth);
  const today   = new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" });
  const content = chart.customContent ?? "";
      ...
      <div className="print-sheet max-w-2xl mx-auto bg-white p-10 shadow-xl border border-gray-100 rounded-2xl print:shadow-none print:border-none print:p-0 print:max-w-none min-h-screen relative overflow-hidden my-8 print:my-0">
        ...
        <div className="relative z-10 flex items-center justify-between mb-6 gap-6">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="display-font text-[17px] font-extrabold uppercase tracking-wider text-teal-900">Customized Dietary Plan</h2>
          </div>
        ...
        {/* Patient Profile Card (Interactive aesthetic) */}
        <div className="relative z-10 grid grid-cols-2 gap-x-8 gap-y-2.5 text-[13px] mb-8 bg-teal-50/20 rounded-xl p-4 border border-teal-100/40">
          ...
          {chart.dietTemplate?.name && (
            <div className="col-span-2 text-[10px] text-gray-400 font-semibold uppercase tracking-wide border-t border-teal-100/30 pt-2.5">
              Target Framework: <span className="text-teal-700">{chart.dietTemplate.name}</span>
            </div>
          )}
        </div>

        {/* Prescribed Diet Plan Container */}
        <div className="relative z-10 mb-8">
          <p className="display-font font-extrabold text-[13px] uppercase tracking-wider text-teal-800 border-b border-gray-100 pb-2 mb-4">Prescribed Food Regimen & Nutrition</p>
          <div className="text-[13.5px] text-gray-800 whitespace-pre-line leading-relaxed pl-1 font-medium">
            {content || <span className="text-gray-400 italic">No diet content recorded.</span>}
          </div>
        </div>

        {/* Additional Gold-Accented Notes Block */}
        {chart.notes && (
          <div className="relative z-10 mb-8 p-4 bg-amber-50/70 border border-amber-200/50 rounded-xl text-[13px] relative overflow-hidden">
            ...
            <p className="text-amber-900/90 whitespace-pre-line leading-relaxed font-medium">{chart.notes}</p>
          </div>
        )}
        ...
        <div className="relative z-10 mt-14 flex justify-between items-end text-[13px]">
          <div>
            ...
            <p className="text-[11px] text-gray-500 font-medium mt-1">Date: {today}</p>
          </div>
        ...
        <div className="relative z-10 mt-10 pt-4 border-t border-gray-100 text-center text-[10px] text-gray-400 font-semibold uppercase tracking-wider">
          Follow this regimen strictly. Any modifications must be discussed with your physician.
        </div>
      </div>
    </>
  );
