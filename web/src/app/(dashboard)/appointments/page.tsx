      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{formatDate(new Date())} · {appointments.length} appointment{appointments.length !== 1 ? "s" : ""}</p>
          </div>
            ...
            {appointments.map((appt) => (
              <div key={appt.id} className="bg-card rounded-xl border border-border p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <Link href={`/patients/${appt.patientId}`} className="font-medium hover:text-primary transition-colors">
